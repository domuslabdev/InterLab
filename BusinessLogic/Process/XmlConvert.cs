...
namespace BusinessLogic.Process
{
...

    {
        ...
        //chimato dal controller o dal servizio
        public static void UploadXml(string xmlfile)
        {
            ...
            try
            {
                var f = new FileStream(string.Format(@"c:\users\ciemm\downloads\{0}.xml", xmlfile), FileMode.Open, FileAccess.ReadWrite);
            ...
            Xml2Text();
            Xml2Db();
            BusinessLogic.Process.getLotto.UpLoadLotto();
            f1.Dispose();
            f1.Close();
        }
